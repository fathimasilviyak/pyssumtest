        ...
        <div class="row">
          ...
          <div class="card">
              <div class="card-header">
                ...
                <a href="{{ route('super-admin.faculties.create') }}"  class="btn btn-success" style="display: inline-block; float: right"><i class="fas fa-user-plus"></i> Create Faculty</a>
              </div>
                                    @if (session('success'))
          ...
          @php ($s_no++)
              @if($user->status == 1)
                    <tr style="word-break:break-all;">
                        ...
                        <a href="{{ route('super-admin.faculties.show', $user->faculty->id) }}"  class="btn btn-info">Show</a>
                            <!-- <button class="btn btn-warning btnProductUpdate" data-product='' data-url="" id=""><i class="fa fa-pencil"></i></button> -->
                            <a href="{{ route('super-admin.faculties.edit', $user->faculty->id) }}"  class="btn btn-warning"> <i class="fa fa-pencil"></i> </a>
                          @if($user->id != auth()->id())
                            <button type="button" class="btn btn-danger"
                                        onclick="event.preventDefault();
                                            ...
                                            document.getElementById('form-delete-{{$user->id}}').submit()
                                                ...
                                                <form style="display:none" id="{{'form-delete-'.$user->id}}" method="post" action="{{route('super-admin.faculties.destroy',$user->id)}}">
                                                     @csrf
                                                     @method('delete')
                                                </form>
                          @endif
              ...
              <tr style="background-color:#FFE0DC">
                        ...
                        <td>
                            ...
                            <!-- <button class="btn btn-warning btnProductUpdate" data-product='' data-url="" id=""><i class="fa fa-pencil"></i></button> -->
                            <a href="{{ route('super-admin.faculties.complete-create',$user->id) }}"  class="btn btn-warning"> <i class="fa fa-pencil"></i> </a>
                          @if($user->id != auth()->id())
                            <button type="button" class="btn btn-danger"
                                        onclick="event.preventDefault();
                                            if(confirm('Are you really want to delete the faculty?')){
